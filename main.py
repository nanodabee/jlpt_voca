...
def extract_vocab_candidates_from_text(text: str) -> list[dict[str, str]]:
    ...
    # 2) kanji-containing word + reading + meaning
    # Meaning may begin immediately with (, （, [, 【 etc. after reading.
    p_explicit_reading = re.compile(
        rf"^([一-龯々]+[ぁ-ゖァ-ヺー・]*)\s+({kana})(?=\s|[（(\[【〔〈《「『])\s*(.+)$"
    )
    ...
    # Allow meaning to start with whitespace OR immediately with a Korean bracket phrase.
    ...
    for line in lines:
        ...
        if mm:
            ...
            # Prevent duplicated word from being swallowed as meaning in "ホテル ホテル 호텔"
            # Existing explicit duplicated kana format is handled below.
