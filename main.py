...
# Learning-material decorations that should never be spoken.
# Examples: ~月, ～か月, -か月, ・単語, （注） etc.
_PRON_EDGE_MARKS_RE = re.compile(r"^[\s~〜～\-‐‑‒–—―・･·•※＊*☆★◇◆○●◎△▲▽▼□■]+|[\s~〜～\-‐‑‒–—―・･·•※＊*☆★◇◆○●◎△▲▽▼□■]+$")
_PRON_BRACKET_NOTE_RE = re.compile(r"[\(\（\[\【][^)\）\]\】]*[\)\）\]\】]")


def clean_pronunciation_text(value: str) -> str:
    """Remove study-list decorations while preserving actual Japanese reading."""
    s = (value or "").strip()
    if not s:
        return ""
    # Remove explanatory bracket notes and edge-only study symbols.
    s = _PRON_BRACKET_NOTE_RE.sub("", s).strip()
    s = _PRON_EDGE_MARKS_RE.sub("", s).strip()
    # Common leading enumeration / separator residue.
    s = re.sub(r"^[・･·:：/／|｜]+", "", s).strip()
    s = re.sub(r"[・･·:：/／|｜]+$", "", s).strip()
    return s


def clean_surface_for_pitch(value: str) -> str:
    """Normalize only non-lexical decorations for Kanjium lookup."""
    return clean_pronunciation_text(value)


@app.post("/api/speech")
def synthesize_speech(req: SpeechRequest):
    raw_word = (req.word or "").strip()
    raw_reading = (req.reading or raw_word).strip()
    word = clean_surface_for_pitch(raw_word)
    reading = clean_pronunciation_text(raw_reading) or clean_pronunciation_text(raw_word)
    if not reading:
        raise HTTPException(status_code=422, detail="발음할 일본어가 없습니다.")
    if len(reading) > 300:
    ...
    if accent is not None:
        ...
        ssml_text = (
            '<speak><prosody rate="100%" volume="medium">'
            f'<phoneme alphabet="x-amazon-pron-kana" ph="{html.escape(pron, quote=True)}">'
            f'{html.escape(word or reading)}'
            '</phoneme></prosody></speak>'
        )
    else:
        # Safe fallback: exact stored reading, without pitch override.
        ssml_text = (
            '<speak><prosody rate="100%" volume="medium">'
            f'<phoneme alphabet="x-amazon-yomigana" ph="{html.escape(reading, quote=True)}">'
            f'{html.escape(word or reading)}'
            '</phoneme></prosody></speak>'
        )
    ...
    try:
        ...
        if stream is None:
            raise RuntimeError("Amazon Polly에서 음원 스트림을 받지 못했습니다.")
        audio = stream.read()
        stream.close()
    except Exception as e:
        raise HTTPException(status_code=502, detail=f"Amazon Polly 음성 생성 실패: {e}")
...
def normalize_import_unicode(value: str) -> str:
    """Normalize compatibility/legacy Unicode forms before vocabulary parsing.

    NFKC converts CJK compatibility ideographs such as 率 to their standard
    equivalents (率), while also normalizing common full-width compatibility
    forms that can appear in copied/PDF text.
    """
...
def extract_vocab_candidates_from_text(text: str) -> list[dict[str, str]]:
    ...
    """
    text = normalize_import_unicode(text)
