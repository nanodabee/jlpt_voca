...
def extract_vocab_candidates_from_text(text: str) -> list[dict[str, str]]:
    # Vocabulary-list oriented extraction. Conservative on prose, but supports:
    # 1) 漢字（かな） 뜻
    # 2) 漢字 かな 뜻
    # 3) かな 뜻 / カナ 뜻  -> reading automatically equals word
    # 4) 3-line format: word / reading / meaning
    # 5) kana-only 2-line format: word / meaning
    lines = [re.sub(r"\s+", " ", x).strip() for x in text.splitlines() if x.strip()]
    found: list[dict[str, str]] = []

    # Kanji + explicit reading
    p1 = re.compile(r"([一-龯々ぁ-んァ-ヶー]+)\s*[（(]([ぁ-ゖァ-ヺー・]+)[）)]\s*[-–—:：]?\s*([가-힣].*)$")
    p2 = re.compile(r"([一-龯々]+[ぁ-んァ-ヶー]*)\s+([ぁ-ゖァ-ヺー・]+)\s+([가-힣].*)$")
    p3 = re.compile(r"^([一-龯々ぁ-んァ-ヶー]+)\s*[\t|｜]\s*([ぁ-ゖァ-ヺー・]+)\s*[\t|｜]\s*([가-힣].*)$")

    # Kana-only word + Korean meaning
    p4 = re.compile(r"^([ぁ-ゖァ-ヺー・]+)\s+(.+)$")
    ...
    for line in lines:
        ...
        if m:
            ...
            if re.search(r"[가-힣]", meaning):
                found.append(candidate(m.group(1), m.group(2), meaning))
            continue
    ...
    # 3-line format: 漢字 / かな / 한국어
    for i in range(len(lines) - 2):
        a, b, c = lines[i:i+3]
        if re.fullmatch(r"[一-龯々ぁ-んァ-ヶー]+", a):
            if re.fullmatch(r"[ぁ-ゖァ-ヺー・]+", b) and re.search(r"[가-힣]", c):
                found.append(candidate(a, b, c))

    # 2-line kana-only format: かな / 한국어
    for i in range(len(lines) - 1):
        a, b = lines[i:i+2]
        if re.fullmatch(r"[ぁ-ゖァ-ヺー・]+", a) and re.search(r"[가-힣]", b):
